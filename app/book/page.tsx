...
function BookingFlow() {
  ...
  const [draftOccasion, setDraftOccasion] = useState<string | undefined>(
    occasion,
  );
  ...
  const [draftChefSlug, setDraftChefSlug] = useState<string | undefined>(
    chefSlug,
  );
  const [draftPackageId, setDraftPackageId] = useState<string | undefined>(
    packageId,
  );
  ...
  const previewOccasionLabel =
    occasions.find((o) => o.id === draftOccasion)?.label ?? "an evening";
  const occasionLabel =
    occasions.find((o) => o.id === occasion)?.label ?? "an evening";
          ...
          <div className='cursor-native rounded-[18px] bg-paper text-ink shadow-[0_40px_100px_-30px_rgba(0,0,0,0.55)] overflow-hidden'>
              ...
              <div key={step} className='animate-[fadein_260ms_ease-out]'>
                {step === "evening" && (
                  <EveningStep
                    ...
                    onNext={(o, d, t) =>
                      goto({ occasion: o, date: d, time: t, step: "chef" })
                    }
                  />
                )}
                {step === "chef" && (
                  <ChefMenuStep
                    ...
                    onNext={(slug, pkgId) =>
                      goto({ chef: slug, packageId: pkgId, step: "payment" })
                    }
                    onChangeChef={() =>
                      goto({ chef: undefined, packageId: undefined })
                    }
                  />
                )}
                {step === "payment" && (
            ...
            {stepIdx > 0 && (
              <div className='flex items-center justify-between border-t border-ink/10 px-6 md:px-10 py-4 bg-ink/[0.02]'>
                ...
                <div className='text-[11px] font-mono uppercase tracking-[0.14em] text-ink/40'>
                  Progress saved
                </div>
              </div>
            )}
...
function StepTitle({
  ...
  return (
    <div className='mb-8'>
      <div className='text-[11px] font-mono uppercase tracking-[0.14em] text-burgundy'>
        {eyebrow}
      </div>
      ...
      {sub && (
        <p className='mt-3 text-[14px] text-ink/65 leading-relaxed'>{sub}</p>
      )}
    </div>
  );
}
...
function FieldLabel({ children }: { children: React.ReactNode }) {
  return (
    <div className='text-[10.5px] font-mono uppercase tracking-[0.14em] text-ink/55 mb-2'>
      {children}
    </div>
  );
}
      ...
      <div className='ml-auto flex items-center border border-ink/10 rounded-md overflow-hidden bg-ink/[0.03]'>
        <button
          ...
          className={cn(
            ...
            ampmVal === "AM"
              ? "bg-burgundy text-cream font-bold"
              ...
              return (
                <button
                  ...
                  <div className='flex items-start justify-between gap-2'>
                    ...
                    {active && (
                      <Check className='h-4 w-4 text-burgundy shrink-0 mt-0.5' />
                    )}
                  </div>
                  <div className='mt-1 text-[12px] text-ink/60 leading-snug line-clamp-2'>
                    {o.blurb}
                  </div>
                </button>
              );
        ...
        <div className='grid gap-5 md:grid-cols-2'>
            ...
            <Popover>
              ...
              <PopoverContent
                ...
                <Calendar
                  ...
                  disabled={(d) => d < today}
                  className={cn(
                    "p-3 pointer-events-auto bg-white text-ink rounded-lg shadow-lg border border-ink/10",
                  )}
                />
              </PopoverContent>
            </Popover>
          ...
          <div>
            ...
            <div className='mt-1.5 text-[11px] text-ink/50'>
              Any time you like — dinner usually starts 18:00–20:30.
            </div>
          </div>
        </div>
      ...
      <div className='space-y-8'>
        {chefLocked && chef ? (
          <div className='flex items-center gap-4 rounded-[12px] border border-ink/10 bg-white p-3 pr-4'>
            ...
            <div className='min-w-0 flex-1'>
              <div className='text-[10.5px] font-mono uppercase tracking-[0.14em] text-ink/50'>
                Your chef
              </div>
                  ...
                  <button
                    ...
                    <div className='p-3'>
                      <div className='flex items-center justify-between gap-2'>
                        ...
                        {active && (
                          <Check className='h-4 w-4 text-burgundy shrink-0' />
                        )}
                      </div>
                  ...
                  <button
                    ...
                    <div className='grid grid-cols-[minmax(0,1fr)_auto] items-start gap-4'>
                      ...
                      <div className='text-right shrink-0'>
                        <div className='font-display font-bold text-[20px] text-ink tabular-nums'>
                          ${p.price}
                        </div>
        ...
        <div className='flex justify-end pt-2'>
          <PrimaryButton
            ...
            onClick={() =>
              selectedChef && selectedPkg && onNext(selectedChef, selectedPkg)
            }>
            Continue to payment
          </PrimaryButton>
        </div>
    ...
    return (
      <div className='max-w-md mx-auto text-center py-4'>
        ...
        <p className='mt-2 text-[13.5px] text-[#6B6B6B]'>
          We need the occasion, date, chef and menu before payment.
        </p>
      ...
      <div className='grid gap-8 md:grid-cols-[minmax(0,1fr)_320px]'>
        <form onSubmit={handleSubmit} className='space-y-6 min-w-0'>
              ...
              {[
                ...
                <label
                  ...
                  <RadioGroupItem
                    value={m.id}
                    id={`pm-${m.id}`}
                    className='border-black/30 text-[#67242B]'
                  />
                  {m.icon}
                  <span className='text-[14px] text-[#111]'>{m.label}</span>
                  <span className='ml-auto text-[11.5px] text-[#8A8A8A]'>
                    {m.note}
                  </span>
                </label>
              ))}
          ...
          {method === "later" && (
            <div className='rounded-lg border border-black/[0.08] bg-black/[0.02] p-4'>
              ...
              <div className='mt-1 text-[13px] text-[#6B6B6B]'>
                The chef confirms within 24 hours. You'll add a card when they
                accept — the date is held tentatively until then.
              </div>
            </div>
          )}
          ...
          <button
            ...
            {processing
              ? "Processing…"
              : `Pay $${deposit} deposit · Book the night`}
          </button>
        </form>
        ...
        <aside className='md:sticky md:top-24 self-start rounded-xl border border-black/[0.08] bg-black/[0.015] p-5'>
          ...
          <div className='mt-4 flex items-baseline justify-between'>
            ...
            <div className='text-[20px] font-semibold text-[#111] tabular-nums'>
              ${pkg!.price}
            </div>
          </div>
          <div className='mt-1 flex items-baseline justify-between'>
            ...
            <div className='text-[13px] font-medium text-[#67242B] tabular-nums'>
              ${deposit}
            </div>
          </div>
        </aside>
  ...
  return (
    <div className='grid grid-cols-[auto_minmax(0,1fr)] items-baseline gap-3'>
      <dt
        className={cn(
          "text-[12px] shrink-0",
          tone === "dark" ? "text-cream/50" : "text-[#8A8A8A]",
        )}>
        {k}
      </dt>
      <dd
        className={cn(
          "min-w-0 text-right text-[13px] truncate",
          tone === "dark" ? "text-cream" : "text-[#111]",
        )}>
        {v}
      </dd>
    </div>
...
function EveningCard({
  ...
  const Field = ({
    ...
    <div>
      <div className='text-[9.5px] font-mono uppercase tracking-[0.18em] text-cream/45'>
        {label}
      </div>
      <div
        className={cn(
    ...
    <div
      ...
      <div className='flex items-center justify-between'>
        ...
        <div className='text-right'>
          <div className='font-display font-bold text-[13px] lowercase text-cream leading-none'>
            love at first sight
          </div>
          <div className='mt-1 text-[9px] font-mono uppercase tracking-[0.2em] text-cream/55'>
            the evening
          </div>
        </div>
      </div>
      ...
      <div className='absolute inset-x-5 bottom-4 flex items-end justify-between gap-3'>
          ...
          {chef ? (
            <>
              <img
                src={chef.portrait}
                alt=''
                className='h-8 w-8 rounded-full object-cover border border-cream/20'
              />
              <div className='min-w-0'>
                <div className='text-[9.5px] font-mono uppercase tracking-[0.18em] text-cream/45'>
                  Chef
                </div>
                <div className='text-[12px] font-medium text-cream truncate lowercase'>
                  {chef.name}
                </div>
              </div>
            </>
        ...
        <div className='text-right shrink-0'>
          ...
          <div
            className={cn(
              "font-display font-bold text-[18px] tabular-nums",
              pkg ? "text-cream" : "text-cream/30",
            )}>
            {pkg ? `$${pkg.price}` : "$—"}
          </div>
        </div>
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className='border-0 p-0 bg-burgundy-deep text-cream max-w-lg max-h-[92vh] overflow-y-auto rounded-[18px] shadow-[0_40px_100px_-20px_rgba(0,0,0,0.8)]'>
        ...
        <div
          ...
          <div className='mt-6 label text-cream/60'>
            the evening is on the calendar
          </div>
          <DialogHeader className='mt-3'>
            ...
            <DialogDescription className='text-cream/70 text-[14px] leading-relaxed pt-3 max-w-sm mx-auto text-center'>
              A confirmation has been sent. {chef.name.split(" ")[0]} will
              follow up within 24 hours — all inside your dashboard.
            </DialogDescription>
          </DialogHeader>
        </div>
        ...
        <div className='px-6 md:px-8 pb-8 -mt-2'>
          <div className='rounded-[14px] border border-cream/15 bg-cream/[0.04] backdrop-blur-sm p-6'>
            <div className='grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-4 border-b border-cream/10 pb-5'>
              ...
              />
              <div className='min-w-0'>
                <div className='font-display font-bold text-[17px] lowercase text-cream truncate'>
                  {chef.name}
                </div>
              ...
              <div className='text-right shrink-0'>
                <div className='text-[10px] font-mono uppercase tracking-[0.14em] text-cream/40'>
                  Booking
                </div>
                <div className='text-[11px] font-mono text-cream/80'>
                  {bookingId}
                </div>
              </div>
            </div>
            ...
            <dl className='mt-5 space-y-3 text-[13px]'>
              ...
              <SummaryRow
                k='Package'
                v={`${pkg.name} · ${pkg.courses} courses`}
                tone='dark'
              />
            ...
            <div className='mt-5 grid grid-cols-[minmax(0,1fr)_auto] items-baseline gap-3 border-t border-cream/10 pt-4'>
              ...
              <div className='font-display font-bold text-xl text-cream tabular-nums'>
                ${pkg.price}
              </div>
            </div>
          </div>
